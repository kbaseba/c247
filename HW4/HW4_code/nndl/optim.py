...
def sgd_nesterov_momentum(w, dw, config=None):
  ...
  v_temp = v
  v = config['momentum'] * v_temp - config['learning_rate'] * dw
  next_w = w + v + (config['momentum'] * (v - v_temp))
...
def adam(w, dw, config=None):
  ...
  # Bias correction
  v_corrected = (1 / (1 - (config['beta1'] ** config['t']))) * config['v']
  a_corrected = (1 / (1 - (config['beta2'] ** config['t']))) * config['a']

  next_w = w - (config['learning_rate'] / (np.sqrt(a_corrected) + config['epsilon'])) * v_corrected
